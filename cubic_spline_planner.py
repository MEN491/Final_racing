...
def main():  # pragma: no cover
    ...
    data = pd.read_csv('./waypoint_2.csv', sep=", ",engine='python')
    ...
    marklen = len(markerarray.markers)
    if lenx < marklen:
        markerarray.markers = markerarray.markers[:marklen-1]    
    ...
    waypoint_pub = rospy.Publisher('/waypoint_visulization', MarkerArray, queue_size=10) #: Publish to drive
    spline_vis_pub = rospy.Publisher('/spline_visulization', Path, queue_size=10) #: Publish to drive
    path_msg = Path()
    ...
    while(True):
        ...
        waypoint_pub.publish(markerarray)
        rate.sleep()
